...
def akaze_ransac(source, target, params):
    """
    Affine registration using AKAZE keypoints and RANSAC.
...
def descriptor_calculation(source, target):
    ...
    source_keypoints, source_descriptors = akaze.detectAndCompute(source, None)
    target_keypoints, target_descriptors = akaze.detectAndCompute(target, None)
...
def matcher(source_keypoints, target_keypoints, source_descriptors, target_descriptors):
    ...
    if len(good_matches) < _MIN_MATCHES:
        raise ValueError("Not enough good AKAZE matches found.")

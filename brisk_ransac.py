...
def brisk_ransac(source, target, params):
    """
    Affine registration using BRISK keypoints and RANSAC.
    ...
    params : dict
        Registration parameters (echo, registration_size, show).
...
def descriptor_calculation(source, target):
    ...
    source_keypoints, source_descriptors = brisk.detectAndCompute(source, None)
    target_keypoints, target_descriptors = brisk.detectAndCompute(target, None)
...
def matcher(source_keypoints, target_keypoints, source_descriptors, target_descriptors):
    ...
    if len(good_matches) < _MIN_MATCHES:
        raise ValueError("Not enough good BRISK matches found.")
